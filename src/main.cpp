    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          if (front_vehicle[0] < 10 || back_vehicle[0] < 10 || bp.average_scores[check_lane] <= 0) {
            next_d = (lane * 4) + 2;
            if (check_lane != lane) {
              bp.target_vehicle_speed = bp.current_lead_vehicle_speed;
            }
          }

          vector<double> wp0 = getXY(car_s+50, next_d, map_waypoints_s, map_waypoints_x, map_waypoints_y);
          vector<double> wp1 = getXY(car_s+100, next_d, map_waypoints_s, map_waypoints_x, map_waypoints_y);
          vector<double> wp2 = getXY(car_s+150, next_d, map_waypoints_s, map_waypoints_x, map_waypoints_y);
          ...
          if (ptsx.size() > 2) {
            ...
            sl.set_points(ptsx, ptsy);

            double target_x = 30;
            double target_y = sl(target_x);
            ...
            for(int i =0; i < 50 - path_size; ++i) {
              if (ref_vel < bp.target_vehicle_speed - accel) {
                ref_vel += accel;
